        ...
        <main className="w-full flex flex-col relative overflow-hidden" style={{background: "var(--color-bg-gradient)"}}>
            {
                blobs.map((blob, index) => {
                    return <BlobContainer key={`${blob}BlobContainer${index}`} duration={randomIntegerInclusive(25, 120)} delay={-45} left={randomIntegerInclusive(0, 100)} width={randomIntegerInclusive(2, 5)} rotate={randomIntegerInclusive(0, 360)} tailwind='absolute top-full' />
                })
            }
            {children}
        </main>
